...
Work one slice at a time. Update this file in place after each slice: check
completed work, record decisions and validation, and replace stale next actions
...
### 2026-07-14 — Slice 1 complete

- Added eight shared adopter fixtures and exact current-behavior expectations
  for policy JSON, queue candidates, merged transition plans, and generated
  workflow routing/setup behavior.
- Fixed init reconciliation so only absent config selects defaults. Parse and
  validation errors now return the existing typed config error; apply with
  `--yes` preserves invalid config and writes no unrelated workflow.
- Validation passed: `gofmt`, `go vet ./...`, `go test ./...`, and
  `git diff --check`. Implementation review found no remaining safe fixes after
  step-level workflow prefilter assertions were added.
- Next: Slice 2, centralizing the authoritative ownership classifier and
  replacing current collision expectations with explicit no-op or failure
  outcomes.

### 2026-07-14 — Slice 2 complete

- Added the authoritative PR ownership table with repository identity and v2
  `work`, `promotion`, `misroutedWork`, `indeterminate`, and `unmanaged` flows.
- Added trusted `baton-managed-issue:v1` records, the `baton:managed` index,
  bounded `legacyFingerprint` reads, and edit/delete repair behavior.
- Routed policy, queue/snapshot acquisition, dashboards, merged and promotion
  history, and transitions through ownership before enrichment or mutation.
- Generated PR policy and transition workflows now cover all targets with one
  conservative prefilter; reconciliation tests detect compiled-config drift.
- Updated adopter fixtures, public JSON contracts, ADR 0004, workflow docs, and
  the v0.6.0 breaking `feat!:` migration note.
- Validation passed: `gofmt`, `go test ./...`, `go vet ./...`,
  `go test -race ./...`, and `git diff --check`. The bounded implementation
  review applied safe lifecycle, ownership-repair, identity, and reporting
  fixes with no remaining decision required.
- Next: Slice 3, removing mutable intake label state from PR merge authority.

### 2026-07-14 — Slice 3 complete

- Work-PR policy now consumes only repository/PR facts, managed-issue ownership,
  configured references, forbidden closing keywords, and commit rules. Only
  configured reference targets are acquired; closing targets are event-local.
- Removed implementation, skip, and all-trivial label gates from PR policy.
  Queue and recommendation retain those label rules, while transition apply
  reacquires ownership before any awaiting-review write.
- Retired `reject_all_trivial_multi_issue_prs` from compiled policy and rendered
  config while accepting and ignoring the old YAML wire field during the v0.6
  migration window. Removed the obsolete label-only GitHub read surface.
- Added body/label invariance, missing-record, pull-request-reference,
  stale-head, transition-preflight, unmanaged-no-client, config migration, and
  JSON/TOON golden coverage. Commit facts are rejected when the current head
  no longer matches the webhook revision.
- Updated ADR 0004, architecture/config/CLI/output requirements, generated
  workflow guidance, the v0.6.0 breaking `feat!:` note, and adopter docs.
- Validation passed: `gofmt`, `go test ./...`, `go vet ./...`,
  `go test -race ./...`, and `git diff --check`. The bounded implementation
  review applied ownership-integrity, referenced-resource, branch-fact, and
  revision-binding fixes; the follow-up found no remaining safe issue.
- Next: Slice 4, deciding delivery storage and fixture-backed contracts without
  changing production promotion authority.

### 2026-07-14 — Slice 4 complete

- ADR 0005 selects one locked, pinned GitHub issue with a mutable checkpoint
  comment and append-only record comments. Exact active pointers replace
  closed-PR enumeration; the trust boundary, concurrency, permissions,
  bootstrap, repair, rollback, shadow-read, and cutover rules are explicit.
- Added the pure `internal/delivery` seam for canonical markers, strict schemas,
  hash-chain and exact-identity verification, bounded completeness, staging
  coverage, plan freshness, reviewed exclusion evidence, strategy-independent
  cursors, and base-integration facts. No GitHub adapter or production reader
  changed.
- Added adapter-shaped and raw transport fixtures for merge, squash, rebase,
  manual-only promotion, explicit exclusion, duplicate/edit/delete corruption,
  stale seals, missed recorder events, and post-seal record changes.
- Review fixes added an exact staging-coverage watermark, required the active
  seal to be the record-chain head, removed append placement from retry
  identity, strengthened comment and human-review identities, and froze raw
  marker bytes so parser/renderer drift cannot self-validate.
- Validation passed: `gofmt`, `go test ./...`, `go vet ./...`,
  `go test -race ./...`, and `git diff --check`. The implementation review with diet lens found the
  delivery seam earned and no generic storage interface or production
  authority was added.
- Next: Slice 5, bootstrap and trusted delivery-record dual writes while the
  existing ancestry reader remains authoritative.

### 2026-07-14 — Slice 5 complete

- Added an optional pinned delivery locator, bounded GitHub transport, strict
  marker reads, repository identity checks, compare facts, and check-suite
  reacquisition.
- Added trusted `delivery-record` and explicitly consented `delivery-bootstrap`
  flows. Bootstrap exposes source facts, inferred relationships, ambiguities,
  and exact records; stale or ambiguous plans block apply and partial writes
  remain visible in `operationReport`.
- Recorder reconciliation repairs every bounded staging-coverage gap, verifies
  ancestry and containment, reuses idempotent retries, backfills durable issue
  ownership, and advances coverage only after exact checkpoint readback.
- Promotion checks are reacquired once after the record/checkpoint/coverage
  batch, including partial-failure cleanup paths. Recorder and policy workflows
  share a queued repository concurrency group so policy cannot observe a
  half-written batch.
- Updated CLI/output/config contracts, generated workflows, bootstrap and
  adopter guidance, architecture/requirements, and the additive `feat:` SemVer
  classification. The existing ancestry reader remains authoritative.
- Validation passed: `gofmt`, `go test ./...`, `go vet ./...`,
  `go test -race ./...`, `staticcheck ./...`, and `git diff --check`. The bounded
  implementation review and diet lens found no remaining issue.
- Next: Slice 6, shadow-read the delivery ledger, cut authority over only on
  complete matching seals, and remove unbounded history reads.

### 2026-07-15 — Slice 6 complete

- Added deterministic promotion seal planning/finalization/resolution,
  structured bootstrap shadow comparisons, exact retry adoption, and an
  explicit `delivery.authority: shadow|sealed` migration gate. Shadow permits
  bootstrap/recording only; sealed enables policy, transition, and
  recommendation authority without an ancestry fallback.
- PR Policy now appends and activates the exact plan bound to PR node/base/head,
  cursor, coverage, included records, and reviewed exclusions. It re-reads
  current PR prose around fact acquisition, verifies ownership for included and
  excluded work, and invalidates prior green checks when promotions overlap.
- Work transitions resolve immutable staged-work records instead of PR bodies.
  Repository snapshots use the bounded active ledger window and fail closed on
  missing, shadow, corrupt, delayed, duplicate, stale, or capped state. Removed
  `FetchPromotionHistoryContext`, its association reader, and the unbounded
  closed-staging-PR scan; dry-run bootstrap/record plans remain repair
  diagnostics.
- Bumped `prPolicyDecision` and `workItemTransitionPlan` to v3, updated adopter
  fixtures/workflows/docs, and classified the cutover/removal as breaking
  `feat!:` within the cumulative pre-1.0 v0.6.0 minor release. Release Please
  retains ownership of version, changelog, manifest, tags, releases, and pinned
  install-target bumps.
- Validation passed: `gofmt`, `go test ./...`, `go vet ./...`,
  `go test -race ./...`, `staticcheck ./...`, and `git diff --check`. The
  implementation/diet review fixed stale webhook prose, incomplete overlap
  invalidation, excluded-work ownership, and ambiguous retry commitment; no
  unearned abstraction was identified.
- Next: Slice 7, consume sealed promotion plans and close delivered issues with
  a cursor-last idempotent transition.

### 2026-07-15 — Slice 7 complete

- Added pure promotion-consumption planning, exact base-integration identity,
  orphan retry adoption, cursor-independent issue selection, and cursor-last
  checkpoint finalization. GitHub's missing squash/rebase method evidence is
  recorded as `unknown` rather than guessed.
- `pr-transition` now preflights the exact merged promotion, active seal,
  plan/cursor digests, bounded store, issue state, and durable ownership. It
  closes open delivered issues, removes the awaiting-review index, appends or
  reuses integration evidence, and commits the cursor last.
- A cursor already naming the exact promotion returns a total no-op before
  delivered-issue reads. Partial issue or record effects remain truthful and
  retryable in `operationReport`, including rate-limit metadata, long-ledger
  bounded discovery, ambiguous creates, human close/reopen, and orphan
  provenance readback.
- Promotion closing references are optional presentation and must exactly
  match the seal when present. Bumped `workItemTransitionPlan` to v4, updated
  generated guidance, public fixtures/contracts, permissions and adopter docs,
  and classified the change as breaking `feat!:` within the cumulative pre-1.0
  v0.6.0 minor release. Release Please retains all generated release ownership.
- Validation passed: `gofmt`, `go test ./...`, `go vet ./...`,
  `go test -race ./...`, `staticcheck@v0.7.0 ./...`, and `git diff --check`.
  The independent implementation/diet review found and verified fixes for
  production retry provenance, retryability reporting, >100-comment appends,
  pre-cursor reopens, orphan digest truthfulness, and stale public guidance;
  no findings remain.
- Next: Slice 8, derive and commit explicit base-to-staging synchronization
  without confusing promotion result topology with pending direct-base work.

### 2026-07-15 — Slice 8 complete

- Added explicit `integrated`, `direct-base-work-pending`, `diverged`, and
  `unknown` facts bound to reviewed genesis, promotion, or synchronization
  evidence. Promotion merge/squash/rebase results do not require result-SHA
  ancestry into staging, while their recorded staging head must remain in the
  staging lineage.
- Delivery Recorder now verifies and records exact same-repository
  base-to-staging merge-commit PRs, preserves active staged work and promotion
  idempotence, rejects stale replay and staging rewrites, and rechecks affected
  promotions. Baton never opens, pushes, merges, or rewrites the sync PR.
- Promotion policy fails pending direct-base work and revalidates exact PR and
  integration revisions even for an existing seal. Snapshot/queue select sync
  before PR or issue work; doctor rejects linear-history and squash/rebase-only
  sync settings while accepting an unprotected staging branch.
- Bumped `prPolicyDecision` to v4, `deliveryRecordPlan` and queue/snapshot to
  v2, and `nextCandidates` to v3. Added producer-backed sync fixtures and kept
  the cumulative breaking `feat!:` pre-1.0 v0.6.0 classification; Release
  Please artifacts remain untouched.
- Validation passed: `gofmt`, `go test ./...`, `go vet ./...`,
  `go test -race ./...`, `staticcheck@v0.7.0 ./...`, and `git diff --check`.
  The independent implementation/diet review drove replay, lineage, stale
  preflight, exact-event, sealed-rerun, and unprotected-branch fixes; no
  decision-level finding remains.
- Next: Slice 9, expand doctor into the complete live adoption compatibility
  gate for workflows, permissions, checks, queues, and managed-file drift.

### 2026-07-15 — Slice 9 complete

- Doctor now authenticates against the live repository and evaluates exact
  default-branch managed content, labels, ownership and delivery readiness,
  enabled workflows, trusted checkout shape, pinned Baton invocations, Actions
  policy, source-bound required checks, merge settings, rules, and active merge
  queues. Unknown organization hosted-runner policy degrades explicitly.
- The public doctor contract is v2 with per-check remediation and
  ready/degraded/blocked semantics. `--repo` and installer-target flags keep
  live adoption and custom initialization reconciliation on one reviewed path;
  blocked doctor results stop adoption guidance.
- Added six adopter fixtures, full authenticated acquisition coverage, golden
  JSON/TOON results, and semantic workflow-template tests. Review fixes made
  private selected-action policy fail closed, local ref diagnostics
  non-authoritative, required checks exact and GitHub-Actions-bound, and custom
  installer reconciliation consistent.
- Validation passed: `gofmt`, `go test ./...`, `go vet ./...`,
  `go test -race ./...`, `staticcheck@v0.7.0 ./...`, and `git diff --check`.
  This remains part of the cumulative breaking `feat!:` pre-1.0 v0.6.0 release;
  Release Please artifacts remain untouched.
- Next: Slice 10, consolidate current ownership and migration guidance, remove
  stale paths, and complete the release-contract audit.

### 2026-07-15 — Slice 10 and goal complete

- Consolidated CLI, config, output, requirements, architecture, skill,
  interactive, generated, and adopter guidance around durable managed-issue
  ownership, PR flow intent, sealed delivery authority, explicit promotion
  completion, and reviewed base-to-staging synchronization.
- The v0.6 adopter note now provides one migration path covering unmanaged PRs,
  the reserved work prefix, delivery bootstrap/cutover, explicit closure,
  synchronization, removed config fields, and blocked doctor remediation. The
  retired fields are decode-only through the v0.6 window and scheduled for
  removal with the next config-schema major.
- Renamed the maintained Coda projections and internal projection helpers to
  their current v3/v2 contracts, added current JSON/TOON goldens, and bumped the
  one-PR dashboard to v2. Merged managed-work readiness now uses its covered
  staged-work record and fails closed when sealed evidence is unavailable.
- Deleted the obsolete promotion-history transport/tests and retained ancestry
  only as bounded bootstrap comparison or synchronization precondition. No
  production promotion or recommendation path falls back to mutable PR bodies
  or unbounded history.
- The final implementation/diet review fixed output-version, workflow-routing,
  historical-doc, and missing-evidence drift; no findings remain. Validation
  passed: `gofmt`, `go test ./...`, `go vet ./...`, `go test -race ./...`,
  `staticcheck@v0.7.0 ./...`, JSON fixture parsing, and `git diff --check`.
- The cumulative public changes remain a breaking `feat!:` pre-1.0 v0.6.0
  minor. Release Please ownership was verified, and its manifest, changelog,
  configuration, tags, releases, and marked pins were not edited manually.
...
```text
Implement docs/GOAL-ADOPTER-COMPATIBILITY.md from the first incomplete slice.
Work one slice at a time, keep the plan updated in place, preserve unrelated
working-tree changes, reproduce each collision before fixing it, validate each
slice, and run the required code review. Do not stage, commit, push, merge,
release, or mutate a live adopter repository unless I explicitly ask.
```
